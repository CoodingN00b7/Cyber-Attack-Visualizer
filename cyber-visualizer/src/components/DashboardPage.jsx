...
const normalizeStatus=(status="")=>{
  ...
  if(upper==="VERIFIED"||upper==="VERIFIED MOBILE LINE"||upper==="VERIFIED TELECOM LINE"||upper==="INVALID NUMBER") return "SAFE";
  return "SAFE";
};
...
const normalizeHistoryRecord=(record,index)=>{
  ...
  return {
    ...
    assessment: record.assessment || record.displayStatus || "",
...
export default function DashboardPage({setIsModalOpen}) {
  ...
  const modal=sel?(()=>{
    ...
    return {safe,score,list,level,rc,gc,source:sel.source||(safe?"Clean":"Unknown"),breach:sel.breachName||(safe?"None":"Unknown"),scanDate:fmtDate(sel.timestamp),assessment:sel.assessment||sel.displayStatus||""};
  })():null;
      ...
      <motion.div variants={fUp} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-4 ${div}`}>
        <div>
          <div className="flex items-center gap-2.5 mb-0.5">
              ...
              <span style={{background:"linear-gradient(90deg,#22d3ee,#0ea5e9,#0369a1)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Dashboard</span>
            </h1>
                    ...
                    {history.map((r,i)=>(
                      <motion.tr key={r.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*.03}} className="tbl-row transition-colors cursor-pointer border-b last:border-0" style={{borderColor:"var(--divider)"}} onClick={()=>setSel(r)}>
                        ...
                        <td className="px-5 py-3">
                          ...
                          {r.breachName&&normalizeStatus(r.status)==="EXPOSED"&&<span style={{fontFamily:"IBM Plex Mono",fontSize:10,color:"var(--text-faint)"}} className="block truncate max-w-[180px]">{r.breachName}</span>}
                        </td>
                        <td className="px-5 py-3">
          ...
          <motion.div initial={{y:40,opacity:0,scale:.96}} animate={{y:0,opacity:1,scale:1}} exit={{y:20,opacity:0}} transition={{type:"spring",stiffness:340,damping:26}} onClick={e=>e.stopPropagation()} className={`modal-card w-full sm:max-w-4xl max-h-[92vh] sm:max-h-[88vh] flex flex-col rounded-[2rem] overflow-hidden border ${modalShellClass}`} style={{boxShadow: dark ? "0 30px 100px rgba(0,0,0,.45)" : "0 30px 100px rgba(15,23,42,.22)",fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div className="flex-none flex items-center justify-between px-5 py-4" style={{borderBottom: dark ? "1px solid rgba(56,189,248,.14)" : "1px solid rgba(3,105,161,.12)",background: dark ? "rgba(15,23,42,.96)" : "rgba(255,255,255,.96)"}}>
              <div className="flex items-center gap-3 min-w-0">
                <span className={`w-2.5 h-2.5 rounded-full shrink-0 blink ${modal.safe?"bg-cyan-500":"bg-rose-500"}`}/>
              ...
              <div className="flex items-center gap-3 shrink-0 ml-4">
                <span className={`hidden sm:inline font-bold px-3 py-1 rounded-full border tracking-wider ${modal.safe?"text-cyan-500 border-cyan-500/30 bg-cyan-500/10":"text-rose-500 border-rose-500/30 bg-rose-500/10"}`} style={{fontFamily:"IBM Plex Mono",fontSize:10}}>{modal.assessment|| (modal.safe?"✓ CLEAN":"⚠ BREACH DETECTED")}</span>
