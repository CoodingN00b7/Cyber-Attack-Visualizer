...
const buildPhoneLookupResult = (identifier, data) => {
    const isValid = Boolean(data?.valid);
    const lineType = String(data?.line_type || data?.lineType || '').trim().toLowerCase();
    const carrier = data?.carrier || 'Unknown Telecom';
    ...
    if (!isValid) {
        return {
            status: 'SAFE',
            assessment: 'INVALID NUMBER',
            identifier,
            type: 'PHONE',
            severityScore: 0,
            source: 'Numverify',
            breachName: 'Number not validated',
            breachDate: '-',
            compromisedData: 'None'
        };
    }

    const verifiedLabel = lineType === 'mobile' ? 'VERIFIED MOBILE LINE' : 'VERIFIED TELECOM LINE';
    const severityScore = lineType === 'mobile' ? 18 : 10;

    return {
        status: 'SAFE',
        assessment: verifiedLabel,
        identifier,
        type: 'PHONE',
        severityScore,
        source: 'Numverify',
        breachName: `Carrier: ${carrier}`,
        breachDate: new Date().toISOString().split('T')[0],
        compromisedData: `Country: ${location}, Line type: ${lineType || 'Unknown'}`
    };
};
    ...
    try {
        ...
        if (normalizedType === "PHONE") {
            if (!API_KEYS.NUMVERIFY) {
                const fallback = {
                    status: 'SAFE',
                    assessment: 'NUMVERIFY UNAVAILABLE',
                    identifier: normalizedIdentifier,
                    type: normalizedType,
                    severityScore: 0,
                    source: 'Numverify',
                    breachName: 'Live phone verification unavailable',
                    breachDate: '-',
                    compromisedData: 'None',
                    fallbackMode: true
                };
            ...
            try {
                const response = await axios.get('https://apilayer.net/api/validate', { params: { access_key: API_KEYS.NUMVERIFY, number: normalizedIdentifier, country_code: 'IN' } });
                const data = response.data;
                const result = buildPhoneLookupResult(normalizedIdentifier, data);
                await logToDatabase(result);
                return res.json(result);
            } catch (err) {
